...
// 取得資料筆數
// $totalRows = $pdo->query("SELECT count(1) FROM products WHERE `categories_sid` = 3 ORDER BY `sid` ASC;")
//     ->fetch(PDO::FETCH_NUM)[0];

// // 查詢所有商品類別為1數據的sql語句
// $sql = "SELECT *  FROM `products` WHERE `categories_sid` = 3 ORDER BY `sid` ASC;";
...
$sql = sprintf(
    "SELECT * FROM `products`  WHERE `categories_sid` = 3 ORDER BY `sid` DESC LIMIT %s, %s",
    ($page - 1) * $perPage,
    $perPage
);
